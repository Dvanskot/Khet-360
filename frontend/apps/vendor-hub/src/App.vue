<template>
  <div class="vendor-portal">
    ...
    <div v-if="loading" class="loading-indicator">
      ...
      <p>Loading vendor portal data...</p>
    </div>

    <main v-else class="vendor-main">
      <div class="dashboard-grid">
        <!-- Active Orders -->
        <div class="orders-section">
          <div class="section-header">
            <h2>Active Purchase Orders</h2>
            <span class="count-badge">{{ orders.length }} Pending</span>
          </div>
          ...
          <div class="invoice-card">
            ...
            <div class="invoice-form">
              <div class="form-group">
                ...
                <select v-model="selectedPo" :disabled="loading">
              ...
              <div class="form-group">
                ...
                <KInput v-model="invoiceAmount" type="number" placeholder="0.00" :disabled="loading" />
              </div>
              <div class="form-group">
              ...
              <KButton variant="primary" @click="submitInvoice" :disabled="loading || !selectedPo.value || !invoiceAmount.value">
...
import { VendorHubService, PurchaseOrder } from '@/services/vendorHubService';
...
const fetchOrders = async () => {
  try {
    loading.value = true;
    error.value = null;
    orders.value = await VendorHubService.getPurchaseOrders();
  } catch (err) {
...
const confirmDelivery = async (id: string) => {
  try {
    await VendorHubService.confirmDelivery(id);
    // Refresh orders after confirmation
    await fetchOrders();
    alert(`Delivery for order ${id} confirmed. Notifying Tenant ERP...`);
  } catch (err) {
    error.value = 'Failed to confirm delivery. Please try again later.';
    console.error('Error confirming delivery:', err);
  }
};

const submitInvoice = () => {
  // In a real implementation, we'd handle file upload properly
  if (selectedPo.value && invoiceAmount.value) {
    VendorHubService.submitInvoice({
      poNumber: selectedPo.value,
      ...
      error.value = 'Failed to submit invoice. Please try again later.';
      console.error('Error submitting invoice:', err);
    });
  } else {
    alert('Please select a PO and enter an invoice amount.');
  }
};
